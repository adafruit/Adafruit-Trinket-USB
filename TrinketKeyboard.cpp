...
void Trinket_Keyboard::pressKey(uint8_t modifiers, uint8_t keycode1 = 0, uint8_t keycode2 = 0, uint8_t keycode3 = 0, uint8_t keycode4 = 0, uint8_t keycode5 = 0, uint8_t keycode6 = 0)
{
...
void Trinket_Keyboard::releaseKey(uint8_t modifiers, uint8_t keycode = 0)
{	
	// searches report for specified keycode and removes it from the report
	if (report_buffer[0] == modifiers) 
	{
		report_buffer[0] = 0;
	}
	for (uint8_t i = 1; i < 8; ++i) 
	{
		if (report_buffer[i] == keycode)
		{
			report_buffer[i] = 0;
		}
	}
...
void Trinket_Keyboard::releaseKeys(uint8_t modifiers, uint8_t* keycodes, uint8_t sz) 
{	
	if (report_buffer[0] == modifiers)
	{
		report_buffer[0] = 0;
